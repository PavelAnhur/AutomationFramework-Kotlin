...
class UIElement(
    ...
    fun waitForClickable(timeout: Long = UIElementConst.DEFAULT_TIMEOUT_IN_SEC) {
        logger.info("waiting for element is clickable: $this")
        WebDriverWait(driver, Duration.ofSeconds(timeout)).until(ExpectedConditions.elementToBeClickable(by))
    }
